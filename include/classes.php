...
//                                                          			\\
//----------------------------------------------------------------------\\
// EDIT-BY:   anas y. bal           https://www.instqram.ml/@anasybal \\
//======================================================================\\
...
class register {
	...
	function generatePassword($length) {
		// Allowed characters
		$chars = str_split("abcdefghijklmnopqrstuvwxyz0123456789._-!@#$%^&*()+{}[]ابتثجحخدذرزسشصطظضعغفقكلمنهوي");
						...
						<div class="message-content" id="blocked'.$row['idu'].'">
							<div class="message-inner">
								...
								<div class="message-avatar" id="avatar'.$row['id'].'">
									...
									<div class="cover-avatar-com">
										<img src="'.permalink($this->url.'/image.php?t=a&w=50&h=50&src='.$row['image']).'"></div>
									</a>
								</div>
								<div class="message-top">
...
class feed {
				...
				<div class="message-content">
					<div class="message-inner">
						<div class="message-avatar" id="avatar-p-'.$row['id'].'">
							...
							<div class="cover-avatar-com">
								<img onmouseover="profileCard('.$row['idu'].', '.$row['id'].', 0, 0, '.$page.');" onmouseout="profileCard(0, 0, 0, 1, '.$page.');" onclick="profileCard(0, 0, 1, 1, '.$page.');" src="'.$profile_img.'"></div>
							</a>
						</div>
						<div class="message-top">
							'.$menu.'
					...
					<div class="message-comment-box-container" id="comment_box_'.$row['id'].'"'.$style.'>
						<div class="message-reply-avatar">
							'.((!empty($this->user)) ? '<div class="cover-avatar-rep"><img src="'.permalink($this->url.'/image.php?t=a&w=50&h=50&src='.(isset($this->page_data['by']) && $this->id == $this->page_data['by'] ? $this->page_data['image'] : $this->user['image']).'"></div>') : '').'
						</div>
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\''.$group['cover'].'\', \''.($group['id'] ?? null).($group['title'] ?? null).'\', \'covers\', 0)" id="'.$group['cover'].'"><div class="cover-image" style="background-position: center; background-image: url('.permalink($this->url.'/image.php?t=c&w=900&h=200&src='.$group['cover']).')">
							</div></a>
							...
							<div class="cover-description">
								<div class="cover-avatar-content">
									<div class="cover-avatar">
										<a onclick="gallery(\''.$page['image'].'\', \''.$page['id'].$page['name'].'\', \'avatars\', 0)" id="'.$page['image'].'"><span id="avatar'.$page['id'].$page['name'].'"><img src="'.permalink($this->url.'/image.php?t=a&w=150&h=150&src='.$page['image']).'"></span></a>
									</div>
								</div>
					...
					<div class="cover-container">
						<div class="cover-content">
							...
							<div class="cover-description">
								<div class="cover-avatar-content">
									<div class="cover-avatar">
										<a onclick="gallery(\''.$profile['image'].'\', \''.$profile['idu'].$profile['username'].'\', \'avatars\', 0)" id="'.$profile['image'].'"><span id="avatar'.$profile['idu'].$profile['username'].'"><img src="'.permalink($this->url.'/image.php?t=a&w=150&h=150&src='.$profile['image']).'"></span></a>
									</div>
								</div>
		...
		$widget =  '<div class="sidebar-container widget-welcome">
						<div class="sidebar-content">
							...
							<div class="sidebar-inner">
								<div class="sidebar-avatar"><a href="'.permalink($this->url.'/index.php?a=profile&u='.$username).'" rel="loadpage"><div class="cover-avatar-said"><img src="'.permalink($this->url.'/image.php?t=a&w=50&h=50&src='.$image).'"></div></a></div>
			...
			foreach($rows as $comment) {
				...
				<div class="message-reply-container" id="comment'.$comment['id'].'">
					...
					<div class="message-reply-avatar" id="avatar-c-'.$comment['id'].'">
						<a href="'.permalink($this->url.'/index.php?a=profile&u='.$comment['username']).'" rel="loadpage"><div class="cover-avatar-rep"><img onmouseover="profileCard('.$comment['idu'].', '.$comment['id'].', 1, 0, 0)" onmouseout="profileCard(0, 0, 1, 1, 0);" onclick="profileCard(0, 0, 1, 1, 0);" src="'.permalink($this->url.'/image.php?t=a&w=50&h=50&src='.$comment['image']).'"></div></a>
					</div>
	...
	function getMessageType($type, $value, $id) {
		...
		switch($type) {
			...
			case "music":
                return '<div class="message-type-general event-music"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/events/music.svg">'.sprintf($LNG['listened'], $value).'</div><div class="message-divider"></div>';
				break;
		...
		if($type == 1) {
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$url = ($window) ? '<a onclick="openChatWindow(\''.$row['idu'].'\', \''.$row['username'].'\', \''.addslashes(realName($row['username'], $row['first_name'], $row['last_name'])).'\', \''.$this->url.'\', \''.$this->url.'/'.$CONF['theme_url'].'/images/icons/'.$icon.'.png\')"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/'.$icon.'.png" class="sidebar-status-icon"> <img src="'.permalink($this->url.'/image.php?t=a&w=25&h=25&src='.$row['image']).'"> '.realName($row['username'], $row['first_name'], $row['last_name']).'</a>' : '<a href="'.permalink($this->url.'/index.php?a=messages&u='.$row['username'].'&id='.$row['idu']).'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/'.$icon.'.png" class="sidebar-status-icon"> <img src="'.permalink($this->url.'/image.php?t=a&w=25&h=25&src='.$row['image']).'"> '.realName($row['username'], $row['first_name'], $row['last_name']).'</a>';
	...
	function getMentions($target, $string) {
		...
		if($query->num_rows > 0) {
			...
			while($row = $query->fetch_assoc()) {
				$output .= '<div class="mention-user" onclick="doMention(\''.htmlspecialchars($target, ENT_QUOTES, 'UTF-8').'\', \''.htmlspecialchars($string, ENT_QUOTES, 'UTF-8').'\', \''.$row['username'].'\')"><img src="'.permalink($this->url.'/image.php?t=a&w=25&h=25&src='.$row['image']).'" width="16" height="16"> <strong>@'.$row['username'].'</strong> '.realName(null, $row['first_name'], $row['last_name']).'</div>';
			}
			$output .= '</div>';
		} else {
			return false;
	...
	function validateMessage($message, $image, $type, $value, $privacy, $group, $page) {
		...
		if($x == 1) {
			...
			$allowedType = array('map', 'game', 'video', 'food', 'music', 'plugin');
